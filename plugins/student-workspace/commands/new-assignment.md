...
2. Ask for (or infer) a short lowercase-hyphenated name, optionally prefixed `hw<N>-` (e.g., `hw2-problem-set`).
3. Create `assignments/<short-name>/` inside that class folder — or `projects/<short-name>/` instead if it's programming work in a programming course.
